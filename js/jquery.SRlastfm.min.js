        ...
        return this.each(function(i, e) {
            ...
            if (display == 'lovedtracks') {
                ...
                if (options.showerror)
                    $e.html('<p>LastFM feed invalid</p>');
                return false;
            }
            var api = 'https://ws.audioscrobbler.com/2.0/?method=' + method + '&user=' + username + '&api_key=' + apikey + '&limit=' + options.limit + '&format=json&callback=?';
            $.ajax({
                type: 'GET',
                url: api,
                dataType: 'json',
                success: function(data) {
                    if (data) {
                        ...
                        if (options.showerror)
                            $e.html('<p>LastFM information unavailable</p>');
                    }
                },
                error: function(data) {
                    if (options.showerror)
                        $e.html('<p>LastFM request failed</p>');
                }
            });
        });
    }
    ...
    var _callback = function(e, display, feeds, options) {
        ...
        if (feeds) {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (display == 'topalbums' || display == 'topartists' || display == 'toptracks')
                ...
                if (options.image) {
                    ...
                    if (imgurl == '')
                        imgurl = options.noimage;
                    if (imgurl != '')
                        html += '<a href="' + url + '" title="Listen to ' + item.name + ' on Last.FM"><img src="' + imgurl + '" alt="' + item.name + '" /></a>'
                }
                if (options.name)
                    html += '<div class="itemName"><a href="' + url + '" title="Listen to ' + item.name + ' on Last.FM">' + item.name + '</a></div>'
                if (options.artist)
                    html += '<div class="itemArtist">' + artist + '</div>';
                if (options.playcount && playcount != null)
                    html += '<div class="itemPlaycount">' + playcount + ' plays</div>'
                html += '</li>';
                if (row == 'odd') {
                    row = 'even';
                } else {
